...
PROPERTY *property_malloc(PROPERTYTYPE proptype, CLASS *oclass, const char *name, void *addr, DELEGATEDTYPE *delegation)
{
	...
	if (sscanf(name,"%[^[][%[^]]]",propname,unitspec)==2)
	{
		...
		if (prop->ptype!=PT_double && prop->ptype!=PT_complex)
			output_error("property_malloc(oclass='%s',...): property '%s' cannot have unit '%s' because it is not a double or complex value",oclass?oclass->name:"<global>", propname, unitspec);
			/*	TROUBLESHOOT
		...
		else 
		{
			if ((prop->unit = unit_find(unitspec))==NULL)
				throw_exception("property_malloc(oclass='%s',...): property '%s' unit '%s' is not recognized",oclass?oclass->name:"<global>", propname,unitspec);
				/*	TROUBLESHOOT
