...
if ! python$PYTHON_VERSION -m setuptools.core 1>/dev/null 2>&1 ; then
    INSTALL brew install python$PYTHON_VERSION-setuptools
    python$PYTHON_VERSION -m setuptools.core >/dev/null || error "unable to install python$PYTHON_VERSION-setuptools"
fi
